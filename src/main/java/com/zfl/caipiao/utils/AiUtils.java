...
public class AiUtils {
    ...
    private static final Integer MIN_BET_COUNT = 5;
    private static final Integer MAX_BET_COUNT = 8;
    private static final Integer MAX_TOKENS = 32678;

    private static final String API_MODEL = "qwen3.7-max";
    ...
    private static final RestTemplate RS = SpringUtils.getBean(RestTemplate.class);

    private static final String API_KEY = "sk-7f120371cb4c4c65bce6858eb98f6ffa";
    ...
    private static String predictLottery(DataProvider dataProvider, CompareDataProvider compareProvider, String systemMsgTemplate,
        ...
        try {
            ...
            if (flag) {
                ...
                systemMsg = systemMsgTemplate.replace("{ycStr}", compareMsg).replace("{list}",
            ...
            HttpEntity<String> entity = buildRequestEntity(systemMsg);
            log.info("开始调用AI预测接口");
            return getContent(entity, 0, flag);
        } catch (Exception e) {
    ...
    private static HttpEntity<String> buildRequestEntity(String systemMsg) {
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.add("Authorization", "Bearer " + API_KEY);
        httpHeaders.add("Content-Type", "application/json");
        Map<String, Object> requestMap = Map.of(
                "model", API_MODEL,
                "messages", List.of(Map.of("role", ROLE_USER, CONTENT_KEY, systemMsg)),
                "stream", false,
                "top_p", 0.6,
                "temperature", 0.3,
                "enable_search", false,
    ...
    private static String getContent(HttpEntity<String> entity, int retryCount, boolean checkContentError) {
        try {
            String answer = RS.postForObject(URL, entity, String.class);
    ...
    private static boolean checkContentError(String content) {
        ...
        if (split.length < MIN_BET_COUNT || split.length > MAX_BET_COUNT) {
            return true;
        }
